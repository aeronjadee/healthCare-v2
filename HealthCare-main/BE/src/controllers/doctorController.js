...
/**
 * 📌 Doctor: Update lab results for an appointment
 * ----------------------------------------------------
 * - Only accessible by a logged-in user with role = doctor
 * - Doctor can update ONLY their own appointment's lab results
 *
...
const updateLabResults = async (req, res) => {
  try {
    ...
    appointment.labResults = labResults ?? null;
    await appointment.save();

    return res.json({ success: true, message: "Lab results updated", data: appointment });
  } catch (error) {
    console.error("Doctor update lab results error:", error);
    return res.status(500).json({ success: false, message: "Internal server error" });
  }
};

module.exports.updateLabResults = updateLabResults;
